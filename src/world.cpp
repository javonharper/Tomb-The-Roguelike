...
std::vector<Enemy*>& World::getEnemyList()
{
    return enemy_list_;
}
...
void World::setTileAsSeen(int x, int y, int z)
{
    this->getMapLevel(z)->setTileAsSeen(x, y);
}
void World::setTileColor(int x, int y, int z, TCODColor color)
{
    level_list_[z]->setTileColor(x,y,color);
}
int World::getWidth()
{
    return width_;
...
void World::incrementTimeStep()
{
    time_step_ =  time_step_ + 1;
}
Map* World::getMapLevel(int level)
{
    return level_list_[level];
}
